...
def image_conversion(inpath, image_path):
    ...
    pil_images = pdf2image.convert_from_path(inpath,
                                             ...
                                             strict=STRICT,
                                             poppler_path=r'C:\Program Files\Release-22.01.0-0\poppler-22.01.0\Library\bin')
...
def searchable_preprocess(filename_pdf):
    ...
    inpath = filename_pdf
    image_path = "./searchable/image_path_searchable/" # image path tetep yang ini nnti masuk ke function
...
def searchable_result_pdf(img_retrieved):
    
    pdf_counter = 0

    for save_pdf in range(0, len(img_retrieved)):
        pdf_filename = open("./searchable/output_path_searchable/homeland_searchablePDF_" + str(pdf_counter) + ".pdf", "w+b")
